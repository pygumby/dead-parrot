...
async def card() -> dict[str, str]:
    """Get the expert agent's card."""
    return {
        "name": EXPERT_AGENT_NAME,
        "description": EXPERT_AGENT_DESCRIPTION,
    }
...
async def ask(body: AskRequest, request: Request) -> dict[str, Any]:
    ...
    response: dict[str, Any] = await request.app.state.temporal_client.execute_workflow(
        workflow="EcbBsExpertAgentWorkflow",
        arg=body.question,
        id=f"{EXPERT_AGENT_NAME}-rest-{uuid.uuid4().hex[:8]}",
