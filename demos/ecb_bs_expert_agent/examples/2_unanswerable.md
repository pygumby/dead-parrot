# Task: Generate an evaluation dataset for a RAG system on ECB BS – Category "Unanswerable" (5 question-answer pairs)
...
- **Expected questions**:
  BS-related inquiries from ECB staff members that are *not* answerable from the attached document.
  The questions should be plausibly in scope, i.e., the kind of BS question an ECB staff member might reasonably ask the agent, but the answer is not contained in the source material.
- **Expected answer**:
...
- Vary the *type* of unanswerability across the pairs. Aim for a mix of:
  - BS topics plausibly in scope but genuinely absent from the document.
  - BS topics adjacent to what the document covers but not directly addressed.
  - BS topics that sound like they should be in the source documents but aren't.
