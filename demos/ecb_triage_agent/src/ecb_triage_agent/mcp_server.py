...
from .constants import TRIAGE_AGENT_DESCRIPTION, TRIAGE_AGENT_NAME
...
@mcp.tool(name=TRIAGE_AGENT_NAME, description=TRIAGE_AGENT_DESCRIPTION)
async def ask(ctx: Context, question: str) -> dict[str, Any]:
    """Ask a question to the triage agent."""
    ...
    response: dict[str, Any] = await temporal_client.execute_workflow(
        workflow="EcbTriageAgentWorkflow",
        arg=question,
        id=f"{TRIAGE_AGENT_NAME}-mcp-{uuid.uuid4().hex[:8]}",
        task_queue=f"{TRIAGE_AGENT_NAME}-queue",
    )
    return response
